...
/*   Updated: 2022/05/20 11:33:43 by mmondell         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// that marks the end of the tree. Only contains a pointer to left child (root).
template <typename T>
class end_node {
  public:
...
class tree_node/* : public node_types<T>::end_node_type*/ {
  ...
  public:
    typedef typename node_types<T>::endt_node 	    end_node_pointer;
    typedef typename node_types<T>::tree_node 	    node_pointer;

  public:
    tree_node() : right(NULL), left(NULL), parent(NULL), is_black(true) {}
